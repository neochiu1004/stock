        ...
        function openSettingsModal() { 
            document.getElementById('modal-system-settings').classList.remove('hidden');
        }
        function closeSettingsModal() { 
            document.getElementById('modal-system-settings').classList.add('hidden'); 
        }
        ...
        async function pasteApiKey(targetId = 'api-key-input') {
            ...
            } catch (err) { showToast("請長按輸入框手動貼上 API 金鑰", "info"); }
        }
        ...
        function logCashChange(newAmount, reason) {
            ...
            // 最多保存 40 筆歷史，避免 LocalStorage 容量爆滿
    ...
    container.innerHTML = html;
        ...
        function renderRealizedProfitsLedger() {
            ...
            container.innerHTML = list.map(item => {
                ...
                return `
                <div class="p-4 bg-surface-base/60 border border-white/5 rounded-2xl flex flex-col gap-2 font-mono text-xs">
                    <div class="flex justify-between items-center border-b border-white/5 pb-2">
                        <span class="font-black text-white text-sm">${item.assetName} (${item.ticker})</span>
                        <span class="text-text-secondary/60 text-[10px]">${item.date}</span>
                    </div>
                    <div class="grid grid-cols-2 gap-y-1.5 text-text-secondary">
                        <div>賣出股數: <span class="text-white font-bold">${item.shares.toLocaleString()} 股</span></div>
                        <div>賣出單價: <span class="text-white font-bold">NT$ ${item.sellPrice.toFixed(2)}</span></div>
                        <div>買入成本均價: <span class="text-white font-bold">NT$ ${item.buyCostPerShare.toFixed(2)}</span></div>
                        <div>估算稅費: <span class="text-white font-bold">手續費: ${item.fee} ｜ 稅: ${item.tax}</span></div>
                    </div>
                    <div class="flex justify-between items-center pt-1 border-t border-white/5">
                        <span class="font-bold text-text-secondary">此筆已實現損益:</span>
            ...
            if (totalLabel) {
                ...
                totalLabel.innerText = `累計實現損益: NT$ ${prefix}${Math.round(sumRealized).toLocaleString()}`;
        ...
        function calculate() {
            ...
            // 【核心修正】計算僅參與再平衡之 active 現值總額，排除未參與資產
            ...
            state.assets.forEach(asset => {
                if (asset.active === false) return; // 確實排除未參與再平衡資產
...
\`\`\``;
        ...
        function addTransactionRecord() {
            ...
            if (editingTxId !== null) {
                ...
                logCashChange(simulatedCash, `編輯 ${asset.name} 異動`);
                ...
                if (linked) {
                    if (type === 'BUY') {
                        simulatedCash -= ((price * shares) + fee);
                    } else if (type === 'SELL') {
                        simulatedCash += ((price * shares) - fee - tax);
                    }
                    ...
                    logCashChange(simulatedCash, `${type === 'BUY' ? '買入' : '賣出'} ${asset.name}`);
                }
